"""Branded HTML email templates for HealthGuard (no images or profile photos)."""
        ...
        <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#ffffff;border:1px solid #d8ded1;border-radius:4px;overflow:hidden;">
          <tr>
            <td style="background:#2f6b4f;padding:20px 24px;">
              <div style="font-size:12px;letter-spacing:0.12em;text-transform:uppercase;color:#cfe3d6;">HealthGuard</div>
          ...
          <tr>
            <td style="padding:16px 24px 24px;border-top:1px solid #e2e8d5;font-size:12px;line-height:1.5;color:#7c8a76;">
              This message was sent by HealthGuard for Irosin, Sorsogon.<br />
...
def verification_email_text(code: str) -> str:
    return "\n".join(
        [
            "HealthGuard — Confirm your email",
            "",
...
def password_reset_email_html(code: str) -> str:
    body = f"""
      <p style="margin:0 0 16px;">Use this one-time code to reset your HealthGuard password:</p>
      <div style="margin:20px 0;padding:16px 20px;background:#f3f7eb;border:1px solid #d8ded1;border-radius:4px;text-align:center;">
...
def password_reset_email_text(code: str) -> str:
    return "\n".join([
        "HealthGuard — Reset your password",
        "",
...
def login_alert_email_html(email: str, ip_address: str | None) -> str:
    ...
    body = f"""
      <p style="margin:0 0 16px;">A successful sign-in was detected for your HealthGuard account.</p>
...
def login_alert_email_text(email: str, ip_address: str | None) -> str:
    return "\n".join(
        [
            "HealthGuard — Login alert",
            "",
            "A successful login was detected for your HealthGuard account.",
            f"Email: {email}",
